...
class Spider(object):
    ...
    def parse(self,li,faculy,degree):
        ...
        if 'www' not in url:
            list1['href'] = 'https://www.une.edu.au' + url
        else:
            list1['href'] = url
        list1['course_name'] = li.xpath("./a//text()")[0]
        if list1['course_name'] == '':
            list1['course_name'] = list1['url'].split("/")[-1].replace("-",'')
        html = self.get_html(list1['href'])
        list1['href'] = ''.join(html.xpath("//*[@id='key-facts']/../following-sibling::div[1]//text()")).strip()
        if list1['href'] == '':
            list1['href'] = ''.join(html.xpath("//div[@class='fact-listing__item fact-listing__item--entry-requirements']/div[@data-audience='domestic']/p//text()")).strip()
        ...
        list1['location'] = ';'.join(html.xpath("//*[text()='Campus']/../following-sibling::ul//text()")).strip()
        list1['ATAR'] = ';'.join(
            html.xpath("//*[text()='Entry requirements']/../following-sibling::ul//text()")).strip().split(":")[1]
        list1['careers'] = ' '.join(html.xpath("//*[text()='Career outcomes']/following-sibling::div//text()")).strip()
