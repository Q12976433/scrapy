    ...
    def main1(self,href,faculty):
        ...
        for ul in uls:
            ...
            for li in lis:
                ...
                list1['degree'] = degree
                print(href)
                list1['course_name'] = li.xpath("./a/text()")[0].strip()
                list1['href'] = li.xpath("./a/@href")[0].replace("\n",'').strip()
                if list1['href'].startswith("//"):
                    list1['href'] = list1['href'].replace("//",'http://')
                ...
                list1['Domestic_full'] = ''.join(html.xpath("//dd[@class='show-domestic']/a/text()"))
                list1['session'] = ';'.join(re.findall("(\d{4})",''.join(html.xpath("//p[@class='ml-lg-auto course-year-links']//text()"))))
                list1['desc'] = ''.join([i.strip() for i in html.xpath("//div[@id='course-overview']/following-sibling::div[1]//text()") if i != ''])
                list1['Entry_requirements'] = ''.join([i.strip() for i in html.xpath("//*[@id='credit-prior-learning']/following-sibling::p//text()") if i != ''])
                ...
                if list1['ATAR']=='' and list1['Duration']=='' and list1['Location'] =='' and list1['Domestic_full']=='' and list1['session']==''and list1['overview']=='':
