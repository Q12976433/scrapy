    ...
    for Faculty,url in zip(Facultys,urls):
        ...
        for h5 in h5s:
            ...
            for course in courses:
                t = threading.Thread(target=parse1,args=(Faculty,degree,course))
                t.start()
                th.append(t)
            [t.join() for t in th]


def parse1(Faculty,degree,course):
    list1 = {}
    ...
    list1['Desc'] = ' '.join([i.strip() for i in html.xpath("//div[@ audience='domestic international']//text()") if
                              i != '' or i != ' ']).strip()
    ...
    if tt == '':
        tt = ''.join(
            html.xpath("//small[@class='summary-footnote --small' and @audience='international']/text()")).strip()
    ...
    list1['Careers'] = '; '.join(
        html.xpath("//h4[text()='Career opportunities']/following-sibling::ul/li//text()")).strip()
    list1['Course_code'] = ''.join(
        html.xpath("//h3[text()='CRICOS code']/following-sibling::div[1]/strong/text()")).strip()
    ...
    if len(h4) == 1:
        list1['Entry_requirements'] = ''.join(
            html.xpath("//h4[text()='Entry requirements']/following-sibling::p/text()")).strip()
    else:
        list1['Entry_requirements'] = ''.join(html.xpath(
            "//h4[text()='Entry requirements']/following-sibling::p[position()<=(count(//div[@class='program-detail']//div[@audience='international']/h4[1]/following-sibling::p)-count(//div[@class='program-detail']//div[@audience='international']/h4[2]/following-sibling::p))]/text()")).strip()
